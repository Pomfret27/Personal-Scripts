...
def get_item(start_date: date, target_date: date, item_list: list):
...
if __name__ == "__main__":
    ...
    # 获取今日物品特征值
    today_a = get_item(cycle_start_date, today_date, item_a[2])
    today_b = get_item(cycle_start_date, today_date, item_b[2])

    # 输出格式："物品名称""特征名称"："特征值"
    print(f"{item_a[0][0]}{item_a[1][0]}：{today_a}")
    print(f"{item_b[0][0]}{item_b[1][0]}：{today_b}")
